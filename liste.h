...
int nbEltList(liste lst); // nombre d'éléments //OK
void createList(liste *p); // renvoie une liste statique //OK
...
void delTailList(liste *p); // supprime l'élément de fin//ok
void delFrontList(liste *p); // supprime l'élément de tête/ok
...
bool emptyList(liste p); // renvoie true si la pile est vide, false sinon//ok
bool fullList(liste p); // renvoie true si la pile est pleine, false sinon//ok
...
void bubbleSortList(liste *p); // trie la liste//ok
